...
std::string determineType(std::string target, std::vector<std::string> variables, std::vector<std::string> types) {
...
void handle_statement(const auto &line,
    ...
    if (value.find("+") != -1) {
        ...
        std::vector<std::string> sumsV = {};
        int sums = 0;
        std::string typing;
        std::string newvartyping;
        if (subs[0].find("[") != -1) {
            for (int ii = 0; ii < variables.size();ii++) {
                std::string tempstr;
                tempstr = subs[0].substr(0,subs[0].find("["));
                if (variables[ii] == tempstr) {
                    newvartyping = types[ii];
                }
            }
            if (newvartyping == "array") {
                for (int ii = 0; ii < variables.size();ii++) {
                    std::string tempstr;
                    tempstr = subs[0].substr(0,subs[0].find("["));
                    if (variables[ii] == tempstr) {
                        if (is_number(arrays[std::stoll(values[ii])][std::stoll(subs[0].substr(subs[0].find("[")+1,subs[0].find("]")-subs[0].find("[")-1))]) != -1) {
                            newvartyping = "int";
                            continue;
                        }
                    }
            ...
            for (int ii = 0; ii < variables.size();ii++) { // need to do type error here
                std::string tempstr;
                tempstr = trim(subs[0].substr(0,subs[0].find("[")));
                if (variables[ii] == tempstr) {
                    newvartyping = types[ii];
                }
            }
            if (newvartyping == "array") {
                for (int ii = 0; ii < variables.size();ii++) {
                    std::string tempstr;
                    tempstr = trim(subs[0].substr(0,subs[0].find("[")));
                    if (variables[ii] == tempstr && subs[0].find("[") != -1) {
                        if (is_number(arrays[std::stoll(values[ii])][std::stoll(subs[0].substr(subs[0].find("[")+1,subs[0].find("]")-subs[0].find("[")-1))]) != -1) {
                            newvartyping = "int";
                            continue;
                        }
        ...
        if (var_exist) {
            ...
            types.push_back(newvartyping);
        }
        if (determineType(trim(name), variables, types) == "array") {
            for (int i = 0; i < subs.size(); i++) {
                typing = determineType(trim(subs[i]), variables, types);
                //sumsV = sumsV + arrays[values[i]];
                for (int j = 0; j < variables.size();j++) {
                    if (trim(subs[i]) == variables[j]) {
                        for (std::string x : arrays[std::stoll(values[j])])
                            sumsV.push_back(x);
                    }
                }
            }
            ...
            for (int i = 0; i < variables.size(); ++i) {
                if (trim(variables[i]) == trim(name)) {
                    if (newvartyping == "array") {
                        arrays[std::stoll(values[i])] = sumsV;
                    }
                    else {
                        values[i].push_back(sums);
                    }
            ...
            for (int i = 0; i < subs.size(); i++) { //in04 after latest still broke here
                typing = determineType(trim(subs[i]), variables, types);
                //sumsV = sumsV + arrays[values[i]];
                if (subs[i].find("[") != -1) {
                    for (int j = 0; j < variables.size(); j++) {
                        if (trim(subs[i].substr(0, subs[i].find("["))) == variables[j]) {
                            //for (int x : arrays[std::stoll(values[j])][std::stoll(subs[0].substr(subs[0].find("[")+1,subs[0].find("]")-subs[0].find("[")-1))])
                            sums = sums + std::stoll(arrays[std::stoll(values[j])][std::stoll(subs[0].substr(subs[0].find("[") + 1, subs[0].find("]") - subs[0].find("[") - 1))]);
                        }
                    }
                }
                else {
                    for (int j = 0; j < variables.size(); j++) {
                        if (trim(subs[i]) == variables[j]) {
                            //for (int x : arrays[std::stoll(values[j])][std::stoll(subs[0].substr(subs[0].find("[")+1,subs[0].find("]")-subs[0].find("[")-1))])
                            sums = sums + std::stoll(values[j]);
                        }
                    }
                }
                if (is_number(subs[i]) == 1) {
                    std::string nani = subs[i];
                    sums = sums + std::stoll(subs[i]);
                }
                else {
                    for (int j = 0; j < variables.size(); j++) {
                        if (trim(subs[i]) == variables[j]) {
            ...
            if (var_exist) { //11/30/2022 here LAST CHANGE LATEST CHANGE
                if (name.find("[") != -1) {
                    for (int i = 0; i < variables.size(); ++i) {
                        if (trim(variables[i]) == trim(name.substr(0,name.find("[")))) {
                            arrays[i][std::stoi(name.substr(name.find("[")+1,name.find("]")-name.find("[")-1))] = std::to_string(sums);
                        }
                    }
                }
                else {
                ...
                for (int i = 0; i < variables.size(); ++i) {
                    if (trim(variables[i]) == trim(name)) {
                        values.push_back(std::to_string(sums));
                    }
                }
            }
        }
        //in05 biggest change to handle_function here!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        /*
            ...
            for (int j = 0; j < subline.size(); ++j) {
                if (subline[j].find("+") != -1) {
                    ...
                    //in05 ongoing
                        ...
                        values.push_back(trim(subline[j]));
                        types.push_back("int");
                        //variables.push_back(subline[j]);
                    }
                    else {
                        if (std::find(variables.begin(), variables.end(), subline[j].substr(0,subline[j].find("["))) == variables.end() && subline[j] != "") {
                            variables.push_back(trim(subline[j]));
                            //here array still empty
                        } else {
                            var_exist = true;
                        }
